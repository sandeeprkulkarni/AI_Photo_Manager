...
from modules.deduplicator import process_duplicates_and_triage
...
async def get_unlabeled_faces():
    """Returns unlabeled faces, filtering out duplicates to minimize scanning fatigue."""
    try:
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
...
# ==========================================
# NEW ENDPOINT: Trigger duplicate photo evaluation job
# ==========================================
@app.post("/api/photos/deduplicate")
async def trigger_deduplication():
    try:
        process_duplicates_and_triage()
        return {"status": "success", "message": "Smart sorting complete."}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
