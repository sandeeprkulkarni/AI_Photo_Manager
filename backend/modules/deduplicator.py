...
def parse_flexible_timestamp(timestamp_str: str) -> datetime:
    ...
    if not timestamp_str:
        return datetime.min
    try:
        # Standardize formatting by substituting spacers
...
def run_deduplication_job(target_folder: str, status_tracker: dict):
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            for idx, group in enumerate(groups):
                ...
                for item in group:
                    ...
                    # Downgrade item to a hidden duplicate variant variant
                    conn.execute("""
                                 UPDATE photos
        ...
        status_tracker["is_processing"] = False# backend/modules/deduplicator.py
import os
import sqlite3
import cv2
import re
